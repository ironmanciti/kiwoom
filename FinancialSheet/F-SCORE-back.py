...
processing_items = net_income_items + net_income_codes + asset_codes + asset_items + cash_flow_codes +\
            cash_flow_items + long_liability_codes + long_liability_items + \
            current_liability_codes + current_liability_items + short_term_loans_codes + short_term_loans_items + \
            income_taxes_payable_codes + income_taxes_payable_items + income_taxes_codes + income_taxes_items + \
...
def f_score(file_list_current, file_list_previous, output_name):
    ...
    fs_cur  = fs_cur[fs_cur['market_group'] != '코스닥시장상장법인']  # 코스닥 제외
    fs_prev = fs_prev[fs_prev['market_group'] != '코스닥시장상장법인']  # 코스닥 제외
